...
public class RendlerScheduler implements Scheduler {
	...
	public void resourceOffers(SchedulerDriver driver,
		...
		for (Offer offer : offers) {
			...
			if (launchedTasks < totalTasks && !crawlQueue.isEmpty()) {
                ...
                //Create the crawl and render tasks with the first task on the queue
                String urlData = crawlQueue.get(0);
                
				System.out.println("Launching task " + taskId.getValue() + " with input: " +
                                   urlData);
                //Task for crawler
	...
	public void statusUpdate(SchedulerDriver driver, TaskStatus status) {
		System.out.println("Status update: task " + status.getTaskId().getValue() +" is in state "
                           + status.getState());
		if (status.getState() == TaskState.TASK_FINISHED || status.getState() ==
            TaskState.TASK_LOST) {
			finishedTasks++;
			System.out.println("Finished tasks: " + finishedTasks);
			if (finishedTasks == totalTasks) {
                //Once the total allowed tasks are finished, write the graph file
                String parentDir = new File(System.getProperty("user.dir")).getParent();
                GraphWriter graphWriter = new GraphWriter();
                graphWriter.
                writeDotFile(
                             parentDir + "/result.dot",
	...
	public void frameworkMessage(SchedulerDriver driver,
        ...
        if (childUrlStr.startsWith("crawl")) {
            ...
            if (childUrlStr.length() >= 2) {
                //Parse and remove whitespace. Add the queue, if not already in the queue to crawl
                ...
                //Create the edges from the source to all child links.  Also create the edges to all
                //related nodes in the graph, siblings, parents, great grand parents, etc
