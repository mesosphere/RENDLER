...
public class RendlerScheduler implements Scheduler {
	...
	public RendlerScheduler(ExecutorInfo executorCrawler,
			ExecutorInfo executorRender) {
		this(executorCrawler, executorRender, 5);
	}

	public RendlerScheduler(ExecutorInfo executorCrawler,
			ExecutorInfo executorRender, int totalTasks) {
		...
		this.completedCrawlQueue = Collections
				.synchronizedList(new ArrayList<String>());
		this.edgeList = Collections
				.synchronizedMap(new HashMap<String, Set<String>>());
		this.urlToFileNameMap = Collections
	...
	public void statusUpdate(SchedulerDriver driver, TaskStatus status) {
		System.out.println("Status update: task "
				+ status.getTaskId().getValue() + " is in state "
				+ status.getState());
		if (status.getState() == TaskState.TASK_FINISHED
			...
			if (finishedTasks == totalTasks) {
				...
				// file
				String parentDir = new File(System.getProperty("user.dir"))
						.getParent();
				GraphWriter graphWriter = new GraphWriter();
				graphWriter.writeDotFile(parentDir + "/result.dot",
						urlToFileNameMap, edgeList);
				driver.stop();
			}
		}
	}
